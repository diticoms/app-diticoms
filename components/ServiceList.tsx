    ...
    <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 min-h-[600px] flex flex-col">
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input 
            type="text" placeholder="Tìm tên, SĐT, KTV, Mã phiếu..." className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500"
            value={filters.searchTerm} onChange={e => setFilters.setSearchTerm(e.target.value)}
          />
        </div>
        <div className="flex gap-2">
          <input 
            type="date" className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm font-semibold"
          ...
          {isAdmin && (
            <select 
              className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm font-semibold"
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        {loading ? (
          ...
          <div className="h-full flex items-center justify-center text-slate-400 font-bold uppercase tracking-widest text-sm">Không có dữ liệu</div>
        ) : (
            ...
            {data.map(item => (
              ...
              >
                <div className="flex gap-4 items-center">
                  <div className={`h-12 w-12 rounded-xl flex flex-col items-center justify-center text-[10px] font-black ${selectedId === item.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>
                    <span>{item.created_at.split('T')[0].split('-')[2]}</span>
                    <span>THG {item.created_at.split('T')[0].split('-')[1]}</span>
                  </div>
                  <div>
                    <div className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{item.customerName} - {item.phone}</div>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase">KTV: {item.technician || 'N/A'}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right flex items-center gap-4">
                  <div>
                    ...
                    {Number(item.debt) > 0 && <div className="text-[10px] font-bold text-red-500 uppercase">Nợ: {formatCurrency(item.debt)}đ</div>}
                  </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-widest">
